...
Our approach adopts the Common Data Model for self-describing scientific data that is widely used throughout the geo-science community.
Key features of the \textbf{xarray} package include label-based indexing and arithmetic, interoperability with the core scientific Python packages (e.g. Pandas, NumPy, Matplotlib), out-of-core computation on datasets that don't fit into memory, a wide range of serialization and I/O options, and advanced multi-dimensional data manipulation tools such as group-by and resampling.
...
\textbf{xarray} provides a range of backends for serialization and IO, including the Pickle, netCDF, OPeNDAP (read-only), GRIB1/2 (read-only), and HDF file formats.
...
For example, time series data includes time\-stamps that label individual points in time, spatial data has coordinates (e.g. longitude, latitude, elevation), and model or laboratory experiments are often identified by unique identifiers.
...
Unlabeled, N-dimensional arrays of numbers, implemented in the SciPy ecosystem by NumPy, are the most widely used data structure in scientific computing.
...
Our core motivation for developing \textbf{xarray} is to provide labeled data tools for N-dimensional arrays that render such questions moot by preserving consistent labels within the tool.
...
\begin{figure}
	...
	\caption{An example of how a dataset (netCDF or \textbf{xarray}) for a weather forecast might be structured.  This dataset has three dimensions: time, y, and x.  \textit{Temperature} and \textit{precipitation} are three-dimensional arrays that have dimensions time, y, and x.  Also included in the dataset are two-dimensional coordinate arrays \textit{latitude} and \textit{longitude}, having dimensions y and x, and \textit{reference time}, a zero-dimensional (scalar) array.}
	\label{fig:dataset_diagram}
\end{figure}

\section*{Implementation and architecture}

NetCDF forms the basis of the \textbf{xarray} data model, providing a natural and portable serialization format.
\textbf{xarray} provides three main data structures: the \verb|Dataset|, the \verb|DataArray|, and the \verb|Coordinate| (Figure \ref{fig:dataset_diagram}).
...
\begin{itemize}
	...
	\item \textit{Arithmetic}: arithmetic on \textbf{xarray} objects is vectorized along named dimensions and coordinates using the underlying N-dimensional array.
	...
	\item \textit{Interactivity with pandas}: A key feature of \textbf{xarray} is the ability to convert to and from pandas objects to interact with the rest of the PyData ecosystem.
	...
	\item \textit{Out-of-core computation}: \textbf{xarray} integrates with dask to optionally support parallel streaming computation on datasets that do not fit into memory.
\end{itemize}
...
\textbf{xarray} is provided with a large test suite comprised of over 900 unit tests.
