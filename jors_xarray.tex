...
Our core motivation for developing \textbf{xarray} is to provide labeled data tools for N-dimensional arrays that render such questions moot. Every operation in xarray both relies on and maintains the consistency of labels.
...
Arrays can share any number of dimensions in common, but they are presumed to share a common coordinate system.
...
Figure \ref{fig:dataset_diagram} illustrates these concepts for an example weather \verb|Dataset|.
...
\begin{itemize}
	...
	\item \textit{Aggregation}: summing a dimension of an \textbf{xarray} object can be done by dimension name instead of an integer axis number.
